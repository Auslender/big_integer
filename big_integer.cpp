...
inline void increase_size(std::vector<int64_t> &a, uint32_t need)
{
    while (a.size() <= need) {
        a.push_back(0);
    }
...
void add(big_integer &a, big_integer const &b)
{
    ...
    for (size_t i = 0; i < b.number.size() || carry; i++) {
        increase_size(a.number, i);
        a.number[i] = (a.number[i] + (i < b.number.size() ? b.number[i] : 0) + carry);
        carry = 0;
        if (a.number[i] >= BASE) {
            a.number[i] -= BASE;
            carry = 1;
...
void sub(big_integer &a, big_integer const &b)
{
    ...
    for (size_t i = 0; i < b.number.size() || borrow; i++) {
        increase_size(a.number, i);
        a.number[i] = a.number[i] - (i < b.number.size() ? b.number[i] : 0) - borrow;
        borrow = 0;
        if (a.number[i] < 0) {
            a.number[i] += BASE;
            borrow = 1;
...
{
    pos = other.pos;
    number = std::vector<int64_t>(other.number);
    return *this;
}
...
int64_t div(big_integer& a, big_integer const& b) {
